...
#ifdef SH_PC_PORT
/* PC port: the active map overlay header is dynamic (DLL maps), so g_MapOverlayHdr
 * redirects through a pointer; each map's header gets a unique symbol via SH_MAP_NAME,
 * and is non-const because the runtime reformatter populates function pointers. */
...
#ifdef SH_MAP_NAME
#define SH_MAP_OVERLAY_HEADER s_MapOverlayHdr SH_MAP_HDR_PASTE(g_MapOverlayHeader_, SH_MAP_NAME)
#else
#define SH_MAP_OVERLAY_HEADER s_MapOverlayHdr g_MapOverlayHeader_default
#endif
#else
extern const s_MapOverlayHdr g_MapOverlayHdr;
#define SH_MAP_OVERLAY_HEADER const s_MapOverlayHdr g_MapOverlayHdr
#endif
